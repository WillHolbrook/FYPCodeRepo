...
def extract_tei_xml_from_root(root_output_folder_path: Path, root_input_folder_path: Path,
                              grobid_client: GrobidClient) -> None:
    ...
    for input_folder_path in root_input_folder_path.glob("*"):
        if input_folder_path.is_dir():
            ...
            extract_tei_xml_from_pdf(input_folder_path, tei_xml_output_folder_path, grobid_client)
...
def extract_txt_from_root(root_output_folder_path: Path, root_input_folder_path: Path):
    for input_folder_path in root_output_folder_path.glob("*"):
        if input_folder_path.is_dir():
            txt_output_folder_path = root_input_folder_path.joinpath(f"./{input_folder_path.name}")
...
def main():
    ...
    root_tei_xml_output_folder_path = Path("../../FilestoreRepo/FTSE100Info/teixmlreports").resolve()
